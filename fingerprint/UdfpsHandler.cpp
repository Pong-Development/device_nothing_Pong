...
void UdfpsHandler::onFingerUp() {
    ...
    ++mRequestId;
    mCondition.notify_all();
    // Serialize the vendor commands so a cancelled wait cannot start capture afterwards.
...
void UdfpsHandler::run() {
    ...
    while (!mStopped) {
        ...
        while (!mStopped && mPending && requestId == mRequestId) {
            ...
            // Do not block the binder thread: pointer-up and cancellation must be able to
            // abort the wait while the panel is resuming from screen-off.
